...
def import_fge_runs(file_path):
    try:
        d = None
        with open(os.path.join(get_project_root(), "templates/fge_runs.json")) as file:
            data = json.load(file)
        ...
        persist(result, 'FGE', "FGE_Runs", 'RefineryInfo', "import", 'FGE_Runs', "Uploaded", 0)

        log("FGE", "import_fge_runs", "The job has completed", level='Info', email=True, emailSubject="FGERuns")
...
if(__name__ == "__main__"):
    import_fge_runs("C:/dev/temp/FGE/fge_runs.xlsx")
