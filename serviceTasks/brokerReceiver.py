...
class BrokerReceiver(object):
    ...
    def __init__(self, queue, server, database, queryToRun, params):
        self.queue = queue
        self.server = server
        ...
        self.processes = list()
        logging.warning('Connecting to {}.{}'.format(self.server, self.database))
    ...
    def receive(self):
        ...
        self.consumer_tag = self.channel.basic_consume(queue=self.queue,on_message_callback=lambda  ch, method, properties, body: callback(ch, method, properties, body, dataAccess), auto_ack=True)
